...
class MRV(VarSelector):
    ...
    """
    def select_variable(self, grid):
        lowest_domain_length = float("inf")
        selected_i, selected_j = None, None
        width = grid.get_width()
        for i in range(width):
            for j in range(width):
                cell_domain = grid.get_cells()[i][j]
                if len(cell_domain) > 1:
                    if len(cell_domain) < lowest_domain_length:
                        lowest_domain_length = len(cell_domain)
                        selected_i = i
                        selected_j = j
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            row_variables_assigned, row_failure = self.remove_domain_row(grid, row, col)            
            col_variables_assigned, col_failure = self.remove_domain_column(grid, row, col)            
            unit_variables_assigned, unit_failure = self.remove_domain_unit(grid, row, col)           

            if row_failure or col_failure or unit_failure:
                return True
          
            Q.update(row_variables_assigned)
            Q.update(col_variables_assigned)
            Q.update(unit_variables_assigned)
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        i, j = var[0], var[1]
        domain = grid.get_cells()[i][j]
        ac3 = AC3()
        for d in domain:
            if grid.is_value_consistent(d, i, j):
                copy_grid = grid.copy()
                copy_grid.get_cells()[i][j] = d
                if ac3.consistency(copy_grid, {var}) == False:
                    rb = self.search(copy_grid, var_selector)
                    if rb!= None:
                        return rb
                copy_grid.get_cells()[i][j] = ""

        return None

file = open('top95.txt', 'r') # Input file with unsolved sudoku puzzles
problems = file.readlines()

running_time_first_available = []
running_time_mrv = []
backtracking_solver = Backtracking()
select_variable_fa = FirstAvailable()
select_variable_mrv = MRV()
ac3 = AC3()
...
for p in problems:
    ...
    # start_fa = time.time()
    # ac3.pre_process_consistency(g_fa)
    # backtracking_solver.search(g_fa, select_variable_fa)
    # close_fa = time.time()
    # running_time_first_available.append(close_fa - start_fa)
  
    # MRV ALGORITHM
    g_mrv.print() #PRINTS THE ORIGINAL UNSOLVED GRID
    start_mrv = time.time()
    ac3.pre_process_consistency(g_mrv)
    solved_grid_mrv = backtracking_solver.search(g_mrv, select_variable_mrv)
    close_mrv = time.time()
    running_time_mrv.append(close_mrv - start_mrv)
    solved_grid_mrv.print() #PRINTS THE SOLVED GRID
